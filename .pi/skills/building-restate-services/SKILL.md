...
| Testing, deployment, server config, Kafka, advanced topics                                  | Use the project-local `restate-docs` CLI                              |
| Code examples and templates                                                                 | `github.com/restatedev/examples`, `github.com/restatedev/ai-examples` |

## Current docs lookup

For Restate docs not covered by bundled references, use the project-local CLI available on `PATH` inside the devshell:

```bash
restate-docs search "<topic>"
restate-docs docs "<id-or-url>" "<focused query>"
```

Use `restate-docs` for deployment, server configuration, Kafka, admin API, and recently changed docs. Prefer bundled `references/<sdk>/api-and-pitfalls.md` for SDK APIs, determinism rules, and testing scaffolds. If `restate-docs` is not found, ask the user to enter `nix develop` and restart pi.
